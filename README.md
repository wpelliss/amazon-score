...
À l'origine, l'extension calculait un score qualité /10 par produit (moyenne pondérée note + avis + prix). La v2.1 a retiré le prix du score, puis la v2.2 a remplacé le score numérique par le système de seuils combinés note + avis décrit ci-dessus.

## v2.2 — Système de seuils (2026-05-30)
- [x] Remplacement du score /10 par des seuils combinés note + nombre d'avis
- [x] Tri bestsellers : rareté en priorité, nombre d'avis en tiebreaker
- [x] Lien bestsellers : libellé "Top des ventes · CATEGORIE"
- [x] Tooltip : note, avis, prix + prochain palier atteignable
- [x] Badge `— (N/A)` en gris si note ET avis indisponibles

## v2.1 — Correctifs production (2026-05-30)
- [x] Formule : note (65%) + avis (35%) — prix retiré du score, affiché en info tooltip
- [x] Bug observer : observer=null après disconnect (fix scroll infini)
- [x] Score null → badge neutre "— (N/A)" au lieu de "A jeter"
- [x] Bouton déclenchement bestsellers (plus d'auto-scroll bloquant)
- [x] Bug lien bestsellers : querySelectorAll + dernier item (sous-catégorie spécifique)
- [x] Tooltip overflow : flip CSS via getBoundingClientRect
- [x] innerHTML → DOM API (textContent) dans les tooltips
- [x] spinInterval leak : try/catch/finally propre dans invisibleScroll
- [x] Attribution : title="AmazonScore" sur les badges, texte overlay
- [x] Affordance : cursor:pointer + ℹ dans le label
- [x] extractNumber() : correction formule 'k' avec deux décimales
- [x] detectPageType() : branche morte '/s?' supprimée
- [x] parseReviewCount() : filtre > 10 (était > 0)
- [x] Libellés tooltip : "65% du score", "35% du score", prix en "(info)"

## v2.0 — Refonte complète (2026-04-10)
- [x] Architecture modulaire (scoring / parser / ui / content)
- [x] Suppression de jQuery (zéro dépendance)
- [x] Parsing DOM résilient (sélecteurs dédiés par type de page + fallbacks)
- [x] Support page produit, recherche, bestsellers
- [x] Badges CSS avec tooltips, animation, tri par score

## v1.2 — Version initiale
- Formule basique, jQuery, code monolithique

### Barème de score (versions ≤ v2.1, historique)
| Score | Label |
|---|---|
| 9+ | Parfait |
| 8+ | Exceptionnel |
...
>
> **Note** : cette analyse a été réalisée sur la base v2.1 (score /10 avec facteur prix). La plupart des points critiques et importants ont depuis été corrigés (v2.1) ou rendus caducs par le passage au système de seuils note + avis (v2.2). Conservée comme trace de conception.
